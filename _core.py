...
class Curve:
    ...
    def _compile(self):
        ...
        if 0 not in p: p[min(p.keys())-1] = p[0] = 0 # assume 0 before first point
        p[self.max_age] = p[max(p.keys())] # extend flat beyond last point
        x, y = zip(*p.items())
        self.interp = interp1d(x, y)
        #self.y = array([interp(x) for x in self.x])
    ...
    def cai(self):
        p = [(0, 0.)] + [(x, (self[x] - self[x-1])/self.period) for x in self.x[1:]]
