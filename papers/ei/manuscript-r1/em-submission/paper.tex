...
\hypersetup{
  ...
  pdfkeywords={WS3, wood supply, forest estate, forest planning, optimization, libCBM, carbon accounting, environmental modeling, Python}
}
...
\begin{abstract}
We describe WS3, an open-source Python framework that integrates Model~I scheduling, carbon accounting, and raster allocation into a single, scriptable workflow for reproducible forest planning analyses. WS3 ingests Woodstock-style inventories, actions, and scenarios; defines an explicit data model; and automates aspatial--spatial conversions so analysts can move from harvest schedules to georeferenced disturbance footprints. A documented linkage to the Canadian Forest Service Carbon Budget Model (libCBM) provides carbon stock and flux estimates for Canada and other libCBM-calibrated jurisdictions.

This paper documents a modular software architecture that decouples inventory ingestion, linear programming (LP) harvest scheduling, carbon accounting, and raster allocation. Users can combine heuristic and LP approaches and extend indicators as needed. We provide a reproducible parity check against Woodstock on a toy dataset and report scaling experiments that characterize runtime and memory envelopes from desktop to cluster deployments. The complete toolchain---source code, configuration files, and deterministic reproduction scripts---is released under the MIT license with an archived Zenodo DOI.

We demonstrate WS3 on five timber supply areas in northeastern British Columbia, with individual inventories spanning 2.27~Mha (TSA~41) to 5.90~Mha (TSA~24) and a cumulative scaling run aggregating all five to 20.40~Mha. Post hoc libCBM runs provide carbon trajectories for alternative schedules, and raster allocation routines produce disturbance footprints for downstream spatial simulators such as SpaDES. The reproduction suite includes notebooks, container-ready scripts, and pinned dependency manifests so practitioners can adapt the workflow to their inventories and verify results against existing pipelines.

WS3 provides an open implementation of workflows otherwise delivered through closed, licensed systems. The framework is intended as reusable informatics infrastructure for decision support under competing sustainability mandates.
\end{abstract}
...
\begin{itemize}
  ...
  \item Reproducibility: deterministic reproduction package (\path{repro}) that regenerates all figures and tables; continuous integration (CI) pipelines run tests and validate examples.
...
\begin{table}[h]
  ...
  \begin{tabular}{p{2.1cm}p{3.4cm}p{6.1cm}}
    ...
    Reusable & Documentation, provenance, validation & ReadTheDocs site; notebooks/examples; continuous integration (CI)–tested \texttt{pytest} suite; deterministic scripts in \path{repro}; submission checklist metadata \\
    \bottomrule
  \end{tabular}
\end{table}
