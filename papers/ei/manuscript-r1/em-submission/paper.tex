...
\hypersetup{
  ...
  pdfkeywords={forest management, decision support systems, forest estate modelling, optimization, simulation, reproducibility}
}
...
\begin{keyword}
forest management \sep decision support systems \sep forest estate modelling \sep optimization \sep simulation \sep reproducibility
\end{keyword}
...
\begin{itemize}
  ...
  \item Archived release: Zenodo DOI \href{https://doi.org/10.5281/zenodo.18716705}{10.5281/zenodo.18716705}
  \item License: MIT
...
\emph{Technology stack.} WS3 is implemented in Python. Optimization problems are formulated by WS3 and solved with the open-source HiGHS \citep{highs} solver by default through the PuLP \citep{pulp} Python LP interface, with optional Gurobi \citep{gurobi} bindings. Geospatial I/O uses rasterio, fiona, and GeoPandas. Documentation is built with Sphinx.
...
\emph{Reproducibility and packaging.} The repository includes examples and a reproduction package (\path{repro}) with pinned requirements and scripts to generate all figures and tables. Versioned releases are archived on Zenodo \citep{ws3_zenodo2025}. WS3 supports interactive (Jupyter notebooks) and batch (Python scripts) workflows.

\subsection{Quality assurance and benchmarking}
\emph{Automated testing.} WS3 includes a pytest suite that exercises the common, core, financial, forest, and optimization modules. The suite is run routinely during development, and style tooling (Black, Ruff) is bundled in \texttt{requirements-dev.txt} and wired into the development Makefile to keep formatting consistent. Beyond unit tests, WS3 has been exercised in multiple operational planning projects over the past decade, providing repeated end-to-end validation of core workflows.
...
\begin{itemize}
  ...
  \item Reproducibility: deterministic reproduction package (\path{repro}) that regenerates all figures and tables; tests validate examples.
...
All figures and tables are generated by scripts in \path{repro}. The \texttt{make\_repro.sh} script provisions an isolated environment, installs pinned dependencies (\texttt{requirements.txt}), and runs \texttt{generate\_diagrams.py}, the spatial allocation workflow (\texttt{generate\_spatial\_allocation.py}), and the sequential demonstration script (\texttt{generate\_case\_study.py}). Outputs are written to \path{figs} and \path{tables}. The exact versions are preserved via the Zenodo-archived release.

\paragraph{FAIR compliance}
Table~\ref{tab:fair-checklist} summarizes the FAIR/software checklist submitted with the manuscript (see supplementary CSV \texttt{fair\_checklist.csv}). WS3 remains Findable, Accessible, Interoperable, and Reusable through its Zenodo DOI archive and indexed repositories, MIT-licensed code/data artifacts, open tabular and geospatial formats plus Woodstock/libCBM linkages, and full reproduction scripts \citep{ws3_zenodo2025}.
...
\begin{table}[h]
  ...
  \begin{tabular}{p{2.1cm}p{3.4cm}p{6.1cm}}
    ...
    \midrule
    Findable & Persistent identifier; indexed repository & Zenodo DOI \href{https://doi.org/10.5281/zenodo.18716705}{10.5281/zenodo.18716705}; tagged GitHub releases; indexed repository metadata \\
    Accessible & License and public access & MIT license (CITATION.cff, LICENSE); public GitHub repository; reproduction package assets bundled in repository \\
    ...
    Reusable & Documentation, provenance, validation & ReadTheDocs site; notebooks/examples; \texttt{pytest} suite; deterministic scripts in \path{repro}; submission checklist metadata \\
    \bottomrule
  \end{tabular}
\end{table}
...
Source code, configuration files, and reproduction assets are archived at Zenodo (DOI: \url{10.5281/zenodo.18716705}). All scripts used to generate the figures and tables are included; reuse is permitted under the MIT License.
