...
\begin{abstract}
We describe WS3, an open-source Python framework that implements Model~I scheduling, carbon accounting, and raster allocation in a single, scriptable workflow for reproducible forest planning analyses. WS3 ingests Woodstock-style inventories, actions, and scenarios; defines an explicit data model; and automates aspatial--spatial conversions so analysts can move from harvest schedules to georeferenced disturbance footprints. A documented linkage to the Canadian Forest Service Carbon Budget Model (libCBM) provides carbon stock and flux estimates for Canada and other libCBM-calibrated jurisdictions.

This paper documents a modular software architecture that decouples inventory ingestion, linear-programming harvest scheduling, carbon accounting, and raster allocation. Users can combine heuristic and solver-backed approaches and extend indicators. We provide a reproducible parity check against Woodstock on a toy dataset and report computational scaling experiments that characterize runtime and memory envelopes across desktop-to-cluster deployments. The complete toolchain---source code, configuration files, and deterministic reproduction scripts---is released under the MIT license with an archived Zenodo DOI.

We demonstrate WS3 on five timber supply areas in northeastern British Columbia, with individual inventories spanning 2.27~Mha (TSA~41) to 5.90~Mha (TSA~24) and a cumulative scaling run that aggregates all five to 20.40~Mha. Carbon trajectories are computed post hoc using libCBM to illustrate outcomes of alternative schedules, and raster allocation routines produce disturbance footprints that can be consumed by downstream spatial simulators such as SpaDES. The reproduction suite includes notebooks, container-ready scripts, and pinned dependency manifests so practitioners can adapt the workflow to their inventories and verify results against existing pipelines.

WS3 provides an open implementation of workflows that are otherwise delivered through closed, licensed systems. The framework is intended as reusable informatics infrastructure for decision support under competing sustainability mandates.
\end{abstract}
...
Forested landscapes now sit at the centre of intertwined climate mitigation, biodiversity, and bioeconomy mandates, and planners are being asked to reconcile wood supply security with measurable reductions in greenhouse-gas emissions across multi-decadal horizons \citep{canadell2022wgiii, griscom2017natural, fao2020fra}. Meeting these expectations demands decision-support systems that can span harvest scheduling, regeneration and silviculture options, spatial footprint analysis, and explicit carbon accounting, while remaining transparent enough for regulators, Indigenous governments, and industrial partners to audit \citep{law2018land, smyth2020climate}. The resulting analytical load has outgrown the ad hoc spreadsheets and siloed software stacks still common in practice, underscoring the need for integrated modelling platforms that support reproducible workflows and uphold open-science norms such as the PERFICT reproducibility principles for predictive ecology \citep{hampton2015tao, lowndes2017path, mcintire2022perfict}.

Strategic forest planning has long relied on operations-research formulations such as Model~I path scheduling and its variants, which underpin allowable-cut analyses and forest estate studies worldwide \citep{johnson1977techniques, gunn2007models, gunn2009some, paradis2013risk}. Model~I is a strata-based linear program in which decision variables assign fractions of aggregated development-type strata to prescriptions over time; this enables large-scale policy analysis but necessarily abstracts away stand identities. In practice these ideas are delivered through proprietary toolchains---Woodstock, Patchworks, JLP (MELA), and Heureka among others---that package data models, solvers, and reporting inside closed desktop environments \citep{remsoft-woodstock, patchworks, jlp_mela, heureka}. While these systems are mature and widely trusted, closed licensing and opaque configuration formats can limit transparency, collaborative extension, and reproducible research workflows increasingly required by regulators and journals \citep{hampton2015tao}.

Open-source platforms have begun to address parts of this gap. Spatial simulation systems such as SpaDES and LANDIS-II, scenario-management frameworks like SyncroSim, and planning prototypes such as PRISM provide modular, shareable tooling \citep{spades, landisii, syncrosim, nguyen2022prism}. These systems excel at spatial dynamics and scenario orchestration but typically require users to supply their own planning modules, solver integrations, and carbon-accounting pipelines to assemble a full decision-support stack \citep{daigneault2022forest, law2018land}. Parallel efforts such as the French CAPSIS platform highlight the promise and complexity of interoperable forest growth modelling ecosystems \citep{fortin2022altframework, fortin2022metamodel, power2015artemis}. This persistent gap between open landscape-simulation scaffolds and purpose-built forest-estate planning motivates the open, reproducible framework we introduce with WS3.

WS3 is a fully open Python framework that preserves the expressive power of legacy Model~I scheduling while embedding reproducibility, automation, and carbon accounting into the workflow. It imports Woodstock-format text files (or equivalent information in \emph{ad hoc} formats via custom Python parsers) to protect historical investments, provides solver-flexible optimization, couples to the Canadian Forest Service libCBM implementation, and supports hybrid spatial–aspatial workflows compatible with geospatial rasters and SpaDES-based simulations \citep{remsoft-woodstock, pulp, highs, gurobi, kurz2009cbm, libcbm_py, spades}. We do not claim a new optimization formulation; rather, we provide a transparent and auditable implementation that can be inspected, extended, and reproduced across institutions \citep{hampton2015tao, lowndes2017path}.
...
The remainder of this article details the WS3 architecture and data model (Section~2), demonstrates reproducible use-case workflows linking solver-backed scheduling to libCBM carbon accounting and spatial disturbance allocation (Section~3), discusses scope and limitations (Section~4), compares WS3 with alternative tools (Section~5), and summarises availability, authorship, and future work to facilitate uptake.
...
\begin{itemize}
  ...
  \item opt: provides a solver-flexible interface to formulate linear programming harvest-scheduling problems (objective, variables, constraints) and to solve them using PuLP/HiGHS by default, with optional Gurobi.
  \item spatial: maps aspatial schedules to rasters (e.g., GeoTIFF) for visualization and spatial policy evaluation (hybrid spatial/aspatial workflow).
...
\emph{Technology stack.} WS3 is implemented in Python and distributed via PyPI. Optimization problems are formulated by WS3 and solved with the open-source HiGHS \citep{highs} solver by default, with optional PuLP \citep{pulp} and Gurobi \citep{gurobi} bindings. Geospatial I/O uses rasterio/fiona/GeoPandas. Documentation (Sphinx) and continuous integration provide API checks, example executability, and unit testing.
...
\label{sec:usecase}
We present a two-stage sequential pipeline that schedules harvest in WS3 and then evaluates carbon dynamics using libCBM. The workflow replicates example 031\_ws3\_libcbm\_sequential-builtin.ipynb to ensure reproducibility.

\emph{Study design.} We use the public example dataset \texttt{tsa24\_clipped}. It is intentionally modest in size so that the full pipeline (scheduling, carbon accounting, and spatial allocation) can be executed and audited on standard hardware, and it mirrors the tutorial dataset distributed with WS3 to ensure transparent, reproducible inputs. Inputs are stored as Woodstock-format text files (landscape, areas, yields, actions, transitions) under \path{examples/data/woodstock_model_files_tsa24_clipped} and imported into a \texttt{ForestModel}. We schedule harvest using a self-parameterizing area-control heuristic to produce an aspatial action schedule over a 100-year horizon (10 periods of 10 years), then export a libCBM Standard Input Table (SIT) to simulate annual carbon stocks and fluxes.
...
Beyond the sequential carbon-accounting demonstration, WS3 includes executable notebooks that act as tutorials and validation exercises, from synthetic cases to policy-oriented analyses. Example 040 implements the Neilson hack and compares aggregated WS3 carbon indicators with libCBM in a no-disturbance scenario. Example 041 illustrates carbon-aware optimization by embedding carbon prices/constraints in the LP and evaluating schedules with \texttt{libcbm\_py}.
...
\noindent
WS3 differs in that it provides an open Python API, a documented libCBM integration, and a hybrid spatial/aspatial workflow intended for reproducible analyses. It is a framework rather than a GUI decision-support suite; spatial adjacency/opening-size constraints and road-network planning are outside its scope. SIMFOR operates at stand level, whereas WS3, Woodstock, Patchworks, Heureka, and JLP target landscape-scale planning with varying degrees of spatial explicitness and availability \citep{remsoft-woodstock, patchworks, spades, heureka, jlp_mela, simfor}.

\paragraph{Comparison narrative}
Proprietary landscape systems such as Woodstock and Patchworks provide mature optimization and spatial planning capabilities, including native support for adjacency and block constraints, but their licensing and closed codebases limit transparent methods development, automated testing, and reproducibility at scale. SpaDES is an open simulation framework oriented to spatial processes and agent-based dynamics; it does not natively provide a solver-backed harvest scheduling layer but can interoperate with WS3 via \texttt{spades\_ws3} for hybrid workflows. Heureka and JLP are landscape-oriented and include optimization, yet are region-specific or legacy-maintained, which constrains adoption beyond their primary jurisdictions.

Carbon-aware planning and optimization has an active literature that couples timber supply models with carbon accounting and evaluates mitigation trade-offs under alternative schedules and policies. Early integrations such as CO2Fix paired with timber supply modelling illustrate carbon-informed planning workflows \citep{neilson2006co2fix}. More recent sector and landscape studies quantify mitigation potential and carbon capacities under harvest scenarios \citep{smyth2020climate, boisvenue2022managing}, and graduate theses in British Columbia explicitly evaluate climate objectives in forest planning optimization contexts \citep{ke2024climate, yan2025mathematical}. WS3 provides a reproducible, open framework for constructing these workflows while keeping the scheduling engine transparent and auditable.

WS3 integrates: (i) a solver-flexible, path-based Model~I LP generator embedded in an open forest estate API; (ii) a documented linkage to libCBM for carbon stocks and fluxes; and (iii) a hybrid spatial/aspatial pipeline with standard geospatial I/O. This design supports reproducible workflows in notebooks and scripts and allows integration into larger research and decision-support ecosystems. Where spatially explicit optimization is required, WS3 can provide schedules and carbon accounting inputs to specialized spatial tools; carbon-related objectives or constraints can be added when users supply coefficients, but they are not required for the core workflow demonstrated here.
