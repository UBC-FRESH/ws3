...
We included a Python script under `scripts/validate_references.py` that uses the `api.crossref.org` REST API to validate the BibTeX references list to make sure they all check out and that DOIs match titles.
...
By default, this grabs references from `papers.bib` and outputs a validation report to `reference-validation-report.txt`.
