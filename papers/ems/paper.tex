...
\emph{Optional scalability experiment.} To characterize scaling on real inventories, the reproduction package optionally integrates a DataLad-hosted benchmark dataset comprising five non-overlapping timber supply areas (TSAs) in northeastern British Columbia (the same study region as \citealt{boisvenue2022managing}). Enabling the optional step (\texttt{RUN\_SCALING=1}) fetches the dataset and executes \texttt{papers/ems/repro/run\_scaling\_benchmarks.py}, which sorts TSAs by complexity, evaluates them individually and cumulatively (\texttt{tsa08}, \texttt{tsa08+tsa40}, \ldots, \texttt{tsa08+tsa40+tsa41+tsa24+tsa16}), and records deterministic runtimes and memory footprints for both single-core (1 worker) and 16-worker runs. Inside the reference container (Ubuntu~24.04, dual Xeon~6254 CPUs, 72 logical cores, 768~GB RAM) single-core runs finish scheduling in 4--63~s and spatial allocation in 15--82~s as problem size grows from one TSA (4.7\,k dtype--age pairs) to the full mash-up (37.7\,k dtype--age pairs). The 16-worker mode leaves the outputs unchanged but roughly halves spatial allocation wall-time on the larger mash-ups (e.g., 82~s to 36~s on the full suite), and peak resident memory remains below 0.7~GB across all cases. A compact summary of these timings and memory measurements is distributed as \texttt{perf\_scaling.csv} alongside the reproduction scripts.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\linewidth]{figs/scaling_schedule_runtime.png}
  \caption{Heuristic scheduling runtime versus problem size (number of development-type/age combinations) for single-core and 16-worker runs.}
...
\begin{figure}[h]
  ...
  \caption{Spatial allocation runtime versus problem size, using annual ForestRaster allocation after applying the heuristic schedule.}
...
\begin{figure}[h]
  ...
  \caption{Peak resident memory usage remains below 0.7~GB even for the five-TSA mash-up.}
  \label{fig:scaling_mem}
\end{figure}

\begin{table}[h]
  \centering
  \caption{Scalability summary for deterministic runs in the reference container.}
  \label{tab:scaling_summary}
  \begin{tabular}{lrrrrr}
    \toprule
    Combo & Workers & $n_{\text{dtype-age}}$ & Sched (s) & Spatial (s) & Peak RSS (GB) \\
    \midrule
    \texttt{tsa08} & 1 & 4\,682 & 4.23 & 15.71 & 0.27 \\
    \texttt{tsa08} & 16 & 4\,682 & 4.10 & 15.77 & 0.28 \\
    \texttt{tsa08+tsa40} & 1 & 10\,453 & 10.51 & 23.06 & 0.33 \\
    \texttt{tsa08+tsa40} & 16 & 10\,453 & 10.45 & 19.64 & 0.46 \\
    \texttt{tsa08+tsa40+tsa41} & 1 & 16\,653 & 19.29 & 30.13 & 0.47 \\
    \texttt{tsa08+tsa40+tsa41} & 16 & 16\,653 & 19.26 & 20.35 & 0.52 \\
    \texttt{tsa08+tsa40+tsa41+tsa24} & 1 & 26\,346 & 41.47 & 55.49 & 0.55 \\
    \texttt{tsa08+tsa40+tsa41+tsa24} & 16 & 26\,346 & 41.44 & 29.78 & 0.62 \\
    \texttt{tsa08+tsa40+tsa41+tsa24+tsa16} & 1 & 37\,691 & 62.11 & 79.46 & 0.63 \\
    \texttt{tsa08+tsa40+tsa41+tsa24+tsa16} & 16 & 37\,691 & 62.30 & 36.10 & 0.69 \\
    \bottomrule
  \end{tabular}
\end{table}
