...
\newcommand{\loadWoodstockParity}{%
  ...
  ]{tables/woodstock_parity_placeholder.csv}{metric=\metricname,ws3_value=\wsvalue,woodstock_value=\woodstockvalue,percent_diff=\percentdiff}{%
    \ifcsvstrcmp{\metricname}{total_harvest_area_kha}{%
      ...
      \ifcsvstrcmp{\metricname}{total_harvest_volume_Mm3}{%
        ...
        \ifcsvstrcmp{\metricname}{total_growing_stock_units}{%
...
\paragraph{Parity with Woodstock (validation placeholder)}
To reassure readers that WS3 implements standard inventory accounting correctly, we include a parity check against a prescriptive Woodstock run using the same toy dataset and an identical action schedule. The CSV artifact \path{papers/ems/tables/woodstock_parity_placeholder.csv} will be populated from a fresh licensed Woodstock run prior to final submission; it reports total harvest area, harvest volume, and growing stock along with WS3-relative percent differences. The expectation is exact or near-exact agreement given identical inputs.

\loadWoodstockParity
\begin{table}[h]
  \centering
  \caption{WS3 vs. Woodstock parity on the toy dataset (identical schedule). Values to be filled from the accompanying CSV artifact.}
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    Total harvest area (kha) & \woodstocknum{\woodstockHarvestAreaWS} & \woodstocknum{\woodstockHarvestAreaWoodstock} & \woodstocknum{\woodstockHarvestAreaDiff} \\
    Total harvest volume (Mm$^3$) & \woodstocknum{\woodstockHarvestVolumeWS} & \woodstocknum{\woodstockHarvestVolumeWoodstock} & \woodstocknum{\woodstockHarvestVolumeDiff} \\
    Total growing stock (units) & \woodstocknum{\woodstockStockWS} & \woodstocknum{\woodstockStockWoodstock} & \woodstocknum{\woodstockStockDiff} \\
    \bottomrule
  \end{tabular}
\end{table}
