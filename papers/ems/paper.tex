...
% Spatial stub (note it's a placeholder)
\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\linewidth]{figs/f3_spatial_stub.png}
  \caption{Spatial allocation schematic (stub). A stylized grid highlights an illustrative \textasciitilde1\% of cells as ‘harvested’ in one period, emphasizing the small spatial footprint typical of long-horizon planning. This placeholder will be replaced with a map derived from the raster allocation in the final submission.}
  \label{fig:spatial_stub}
\end{figure}
...
\emph{Reproducibility.} This case is intended to be transparent and portable; data are included in the repository’s examples folder. The spatial allocation step is kept illustrative; WS3’s hybrid spatial/aspatial workflow supports allocation of aspatial schedules to raster inventories, and we will provide a finalized spatial figure in the submission package. The folder papers/ems/repro contains a make\_repro.sh script that creates a virtual environment, installs pinned dependencies, and runs a deterministic Python script to regenerate Figures~\ref{fig:flows}--\ref{fig:cbm} and associated CSV tables.

\subsection*{Broader example library and carbon-aware optimization}
Beyond the sequential carbon accounting case, WS3 includes a curated set of executable notebooks designed as progressive tutorials, from synthetic examples to realistic policy-oriented analyses. Example 040 showcases carbon-aware optimization by embedding carbon prices and/or constraints directly in the harvest scheduling LP and computing carbon stocks and fluxes with \texttt{libcbm\_py}. A public, end-to-end case study exploring carbon-aware scheduling in a stylized British Columbia landscape is available in the repository by Yan (MSc), including scenario automation, sensitivity analysis, and reproducible outputs \citep{yan2025mathematical, yan_repo}.
