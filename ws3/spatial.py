...
class ForestRaster:
    ...
    """
    _hdt_map: dict[int, tuple[str, ...]]
    _hdt_func: Callable[[tuple[str, ...]], int]
    _acodes: list[str]
    _acode_map: dict[str, str]
    _forestmodel: Any  # ForestModel instance
    _horizon: int | None
    _base_year: int
    _period_length: int
    _time_step: int
    ...
    _a2i: dict[str, int]
    _p: int
    _src: rasterio.DatasetReader
    _x: np.ndarray
    _ix_forested: tuple[np.ndarray, ...]
    _blkid: np.ndarray
    _d: float
    _pixel_area: float
    _piggyback_acodes: dict[str, list[tuple[str, float]]] | None
    _snk_path: str
    _tif_dtype: rasterio.dtype
    _tif_compress: str
    _snk: dict[tuple[int, int], dict[str, rasterio.DatasetWriter]]
    _snkd: dict[tuple[str, int], np.ndarray]
    _is_valid: bool
    _disturb_thresh: int

    def __init__(
        self,
        hdt_map: dict[int, tuple[str, ...]],
        hdt_func: Callable[[tuple[str, ...]], int],
        src_path: str,
        snk_path: str,
        acode_map: dict[str, str],
        forestmodel: Any,
        base_year: int,
        horizon: int | None = None,
        period_length: int = 10,
        tif_compress: str = 'lzw',
        tif_dtype: rasterio.dtype = rasterio.uint8,
        piggyback_acodes: dict[str, list[tuple[str, float]]] | None = None,
        time_step: int = 1,
        disturb_thresh: int = 10,
    ) -> None:
        """
        ...
        *clearcut* disturbance are disturbed by a piggybacked *slashburn*
        ...
        self._i2a = dict(enumerate(self._acodes))
    ...
    def allocate_schedule(
        ...
        verbose: bool = False,
        commit: bool = True,
        sda_mode: str = 'randblk',
        da: int = 0,
        ovrflwthr: int = 0,
        fudge: float = 1.0,
        nthresh: int = 0,
        minage: int = 1,
        aggregate_disturbance: bool = False,
    ) -> None:
        """
    ...
    def __enter__(self) -> ForestRaster:
        """The value returned by this method is assigned to the variable after ``as``."""
        return self

    def __exit__(self, exc_type: Any, exc_value: Any, exc_traceback: Any) -> bool:
        """Returns either True or False. Don't raise any exceptions in this method."""
        self.cleanup()
        return True
    ...
    def _write_snk(self, write: bool = True) -> None:
        """Write all sink rasters for the current period."""
        for dy in range(0, self._period_length, self._time_step):
            for acode in self._acodes:
    ...
    def _init_snkd(self) -> None:
        """Initialize the sink data dictionary for the current period."""
        self._snkd = {
            (acode, dy): np.full(self._x[0].shape, 0, dtype=self._tif_dtype)
            for dy in range(0, self._period_length, self._time_step)
            for acode in self._acodes
        }
    ...
    def _transition_cells(
        ...
        if c > 1. and verbose > 1:
            print('missing area:', acode, dy, tarea - xa, from_dtk)
        c = min(c, 1.)
        ...
        if mode == 'randpxl' or n <= nthresh:
            ...
            raise AssertionError() # no valid mode specified
        return missing_area
    ...
    def _transition_cells_randpxl(
        ...
        """Transition cells using random pixel selection mode."""
    ...
    def _transition_cells_randblk(
        ...
        """Transition cells using random block selection mode."""
        import scipy
        _n = 0
    ...
    def grow(self) -> None:
        """Grow trees, only increment non-NA values."""
        self._p += 1
        # HACK! #############
